-- ============================================================================
-- UNEXPLAINED-RARITY SWEEP, RULE 2: A SINGLE FANTASTIC WITH ZERO NORMAL CRAFTS (owner order, 2026-08-03).
--
-- The hourly sweep's ratio rule needs min_top = 10 before it looks at a family, because at small counts a
-- lucky roll is indistinguishable from a ratio spike. That floor also means an injector who mints UNDER ten
-- fantastics per family per hour never appears at all. The owner's instruction: flag any client-reported
-- FANTASTIC growth whose family shows ZERO client-reported NORMAL growth -- "even if that is only a single
-- fantastic". Crafting cannot do that except by one specific fluke: to roll a fantastic you make a craft,
-- and every craft yields an item, overwhelmingly normal -- so the only honest zero-normal fantastic is a
-- player whose ONLY craft of that family in the whole window came up fantastic (0.16% per roll at the
-- capped rate). That fluke is accepted noise by explicit owner decision; the sweep stays SHADOW-ONLY, so a
-- false positive costs one review row, not a clamp.
--
-- Precisely per the instruction, the rule reads FANTASTIC vs NORMAL -- not the ratio rule's top
-- (supreme+fantastic) vs base (normal+rare) buckets. Two consequences, stated rather than hidden:
--   * A single injected SUPREME with no siblings stays invisible (the ratio rule still catches supremes
--     in volume, >= min_top per family-hour).
--   * An injector who mints a fantastic plus a few RARES as camouflage passes this rule (normal stays 0 --
--     wait, no: rares do not raise norm_cr, so they do NOT help; only faking NORMAL growth evades it, and
--     normal growth is exactly what the ratio rule then counts against them at volume).
--
-- Everything else -- the delta watermark, the server-credit subtraction, the one-signal-per-user-family-day
-- dedupe, the single-embed Discord digest, the watermark-before-webhook ordering -- is unchanged from
-- 20260731200000 / 20260731220000. The function body below is verbatim from 20260731220000 except:
--   1. `d` extracts the exact rarity suffix (rar) instead of a boolean is_top;
--   2. `agg` adds fant_cr / norm_cr buckets beside top_cr / base_cr;
--   3. the WHERE gains the zero-base arm;
--   4. the signal detail records which rule fired plus the fantastic/normal figures, and the digest field
--      says "with ZERO normal crafted" for zero-base rows.
-- The return shape is unchanged (create or replace cannot alter it, and the cron job + review queries
...
-- Fake exactly ONE fantastic of growth on an account with no normal growth in that family, sweep, and
-- expect the zero_base row. The old code cannot flag this (min_top = 10), so this doubles as the
-- prove-the-guard-fails test: run it BEFORE applying and expect zero rows, run it AFTER and expect one.
--
--   begin;
...
--        where item_key ~ '_fantastic$' and earned_total >= 1 limit 1);
--   select * from public.item_unexplained_sweep(false);
--   -- EXPECT (after this migration): one row for that user/family, and clamp_signals detail
--   -- {"rule":"zero_base","fantastic_client_reported":1,"normal_client_reported":0,...},
--   -- plus one amber Discord embed reading "with ZERO normal crafted in the family".
--   rollback;
--
-- NOTE the pg_net caveat from 20260731220000 applies: the Discord message is posted from a background
-- worker and survives the rollback -- expected, and the signal row + watermarks do roll back.
--
-- The cron job needs no change; it calls item_unexplained_sweep(false) by name and picks this body up
-- on its next hourly run. The 24h per-user-per-family dedupe means an account already flagged by the
-- ratio rule today will not double-ping when this lands.
