...
Deno.serve(async (req) => {
  ...
  try { const { data: _cl } = await admin.rpc("is_clamped", { p_user: userId, p_surface: "guild" }); if (_cl === true) return json({ ok: false, clamped: true, error: "This account is restricted from guild features." }); } catch { /* clamp check unavailable -> allow */ }
