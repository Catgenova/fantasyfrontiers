...
Deno.serve(async (req) => {
  ...
  try { const { data: _over } = await admin.rpc("rl_hit", { p_subject: user.id, p_bucket: "marketplace", p_limit: 60, p_window_secs: 60 }); if (_over === true) return json({ ok: false, error: "Too many requests." }, 429); } catch { /* limiter unavailable -> allow */ }
  // Clamp gate: an account quarantined from the marketplace is refused (see migration 20260724210000).
  // Fail-open like the rate limiter -- a check that is DOWN must never lock a legit player out of trading.
  try { const { data: _cl } = await admin.rpc("is_clamped", { p_user: user.id, p_surface: "marketplace" }); if (_cl === true) return json({ ok: false, clamped: true, error: "This account is restricted from the marketplace." }); } catch { /* clamp check unavailable -> allow */ }
