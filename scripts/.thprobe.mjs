...
// PROBE: fixed set/cloak, legendaries from SIM_LEGS, so only the knob changes between runs.
const LEGS = (process.env.SIM_LEGS || BUILD.legs.join(",")).split(",").filter(Boolean);
const SET = process.env.SIM_SET || BUILD.setLayers[0];
for (const leg of LEGS) await runOne(SIM_CLASS + ": " + leg, { leg, setLayer: SET, cloakLeg: "d2_ruin" }, DURATION_MS);
